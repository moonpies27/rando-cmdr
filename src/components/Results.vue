  ...
  <div class="mb-12">
    ...
    <div class="flex items-center justify-center mt-6 text-center space-x-4 flex-shrink-0 flex-wrap text-gray-700">
      <div v-for="(player, index) in players" :key="index">
        ...
        <img v-if="imgSrc(index)" :src="imgSrc(index)" class="w-56 h-72 mx-auto"/>
        <svg v-else class="fill-current" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"></path>
        </svg>
        <a v-if="cardTitle(index)" :href="scryfallURI(index)" target="_blank" class="underline">{{ cardTitle(index) }}</a>
        <p v-else>Waiting to spin!</p>
      </div>
    </div>
  </div>
</template>
